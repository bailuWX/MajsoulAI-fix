...
class AIWrapper(sdk.GUIInterface, sdk.MajsoulHandler):
        ...
        if self.AI_state == State.WaitingForStart:
            ...
            if op == 'D':
                ...
                if dt21 > 5.0:
                    ...
                    self.writeTimeAitoLong(time11 + "  ai切时间：" + str(dt21) + " 秒")
                # 出牌
                ...
                dt23 = time.time() - self.lastSendTime
                print("ai思考立直: ", round(dt23, 2), "秒")
                if dt23 > 5.0:
                    time11 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                    self.writeTimeAitoLong(time11 + "  ai立直时间：" + str(dt23) + " 秒")
                # 宣告自己立直
                self.on_Liqi(d)
    ...
    def on_JOIN(self, msg_dict):
        ...
        self.send(b'<GO type="201" lobby="0" gpid="EE26C0F2-327686F1"/>\x00')
        # step 4: 用户信息
        self.send(('<UN n0="' + quote('tst-tio') + '" n1="' + quote('user1') + '" n2="' + quote('user2') + '" n3="' +
                   quote(
                       'user3') + '" dan="11,11,11,11" rate="985.47,1648.57,1379.50,1500.00" sx="M,M,M,M"/>\x00').encode())
        # step 5: fake录像地址
        self.send(
    ...
    def newRound(self, chang: int, ju: int, ben: int, liqibang: int, tiles: List[str], scores: List[int],
        ...
        self.send(('<INIT seed="' + ','.join(str(i) for i in seed) + '" ten="' + ','.join(str(i)
                                                                                          for i in
                                                                                          ten) + '" oya="' + str(
            oya) + '" hai="' + ','.join(str(i) for i in self.hai) + '"/>\x00').encode())
        if len(tiles) == 14:
            # operation TODO
    ...
    def on_ChiPengGang(self, msg_dict):
        ...
        if 'type' not in msg_dict:
            # 无操作
            dt22 = time.time() - self.lastSendTime
            print("ai决定不鸣牌: ", round(dt22, 2), "秒")
            if dt22 > 5.0:
                time11 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                self.writeTimeAitoLong(time11 + "  ai决定不鸣牌：" + str(dt22) + " 秒")
        ...
        if type_ == 1:
            # 碰
            dt22 = time.time() - self.lastSendTime
            print("ai思考碰: ", round(dt22, 2), "秒")
            if dt22 > 5.0:
                time11 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                self.writeTimeAitoLong(time11 + "  ai思考碰：" + str(dt22) + " 秒")
            ...
            # 明杠
            dt22 = time.time() - self.lastSendTime
            print("ai思考自杠: ", round(dt22, 2), "秒")
            if dt22 > 5.0:
                time11 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                self.writeTimeAitoLong(time11 + "  ai思考自杠：" + str(dt22) + " 秒")

            self.actionChiPengGang(sdk.Operation.MingGang, [])
        elif type_ == 3:
            # 吃
            dt22 = time.time() - self.lastSendTime
            print("ai思考吃牌: ", round(dt22, 2), "秒")
            if dt22 > 5.0:
                time11 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                self.writeTimeAitoLong(time11 + "  ai思考吃牌：" + str(dt22) + " 秒")
            ...
            # 暗杠
            dt22 = time.time() - self.lastSendTime
            print("ai思考暗杠: ", round(dt22, 2), "秒")
            if dt22 > 5.0:
                time11 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                self.writeTimeAitoLong(time11 + "  ai思考暗杠：" + str(dt22) + " 秒")
            self.actionChiPengGang(sdk.Operation.MingGang, [])
        elif type_ == 5:
            # 加杠
            dt22 = time.time() - self.lastSendTime
            print("ai思考加杠: ", round(dt22, 2), "秒")
            if dt22 > 5.0:
                time11 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                self.writeTimeAitoLong(time11 + "  ai思考加杠：" + str(dt22) + " 秒")
            self.actionChiPengGang(sdk.Operation.JiaGang, [])
        elif type_ == 6:
            # 点炮胡
            dt22 = time.time() - self.lastSendTime
            print("ai思考点炮胡: ", round(dt22, 2), "秒")
            if dt22 > 5.0:
                time11 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                self.writeTimeAitoLong(time11 + "  ai思考点炮胡：" + str(dt22) + " 秒")
            self.actionHu()
        elif type_ == 7:
            # 自摸胡
            dt22 = time.time() - self.lastSendTime
            print("ai思考自摸!: ", round(dt22, 2), "秒")
            if dt22 > 5.0:
                time11 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                self.writeTimeAitoLong(time11 + "  ai思考自摸!：" + str(dt22) + " 秒")
            self.actionZimo()
        else:
            raise NotImplementedError
