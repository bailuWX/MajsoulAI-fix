...
def connect_and_play(ai_obj, opponent_class, user_id, username, lobbytype, gametype, logger_obj, drawer=None):
    client = TenhouClient(ai_obj, opponent_class, user_id, username, lobbytype, gametype, logger_obj, drawer)
    client.connect()
    try:
...
def run_example_ai():
    ...
    game_type = '141'      # '137' 南 '193' 东速高   '201'南速高    '41'四人南可以断19有红宝牌龟速出牌超级难度房间

    logger_obj = Logger("log1", user_name)      # two arguments: id of your test epoch, user name

    connect_and_play(ai_obj, opponent_class, user_id, user_name, '0', game_type, logger_obj)  # play one game
