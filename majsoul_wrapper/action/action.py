...
def get_tile_list(list1: List[Tuple[str, Tuple[int, int]]]):  # 去除手牌的坐标只显示手牌英文名
    list2 = []
    for i in list1:
        list2.append(i[0])
...
def get_format_tile(listp: list) -> list:
    # 东南西北白发中(z1~z7)
    list2 = []
    list1 = listp
    for i in list1:
        num = i[0]
        style = i[1]
        if style == 'm' and num != '0':
            list2.append(num + "万")
        if style == 's' and num != '0':
            list2.append(num + "条")
        if style == 'p' and num != '0':
            list2.append(num + "桶")
        if i == '1z':
            list2.append("东风")
        if i == '2z':
...
class GUIInterface:
    ...
    def actionDiscardTile2(self, tile: str):  # 备用方案,不过我没用
        ...
        if len(L) == 0:
            ...
            if len(L) == 0:
                L = self._getHandTiles()
                time.sleep(0.2)
        print("找牌用时:" + str(time.time() - time1))
        len1 = len(L)
        print("找不到的牌：" + tile + ",手牌数量:" + str(len1) + "-1")

        index1 = random.randint(0, len(L))
        x1 = L[index1][1][0]
        y1 = L[index1][1][1]
        tile1 = L[index1][0]
        # tile = tile1
        print("随机出的牌为:" + tile1 + ":" + str(x1) + "," + str(y1))
        pyautogui.moveTo(x=x1, y=y1)
        time.sleep(0.3)
        pyautogui.click(x=x1, y=y1, button='left')
        time.sleep(1)
        # out of screen
        # pyautogui.moveTo(x=x1, y=(y1 - 100))
        raise Exception(
            'GUIInterface.discardTile2 tile not found. L:', L, 'tile:', tile)
    ...
    def clickCandidateMeld(self, tiles: Tuple[str]):
        # 有多种不同的吃碰方法，二次点击选择
        assert (len(tiles) == 2)
